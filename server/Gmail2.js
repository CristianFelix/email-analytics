...
var retrieve = function (key, q, endpoint, opts, obj) {
  ...
  if(obj.nextPageToken){
    console.log("Getting next page token")
    p = "?pageToken="+ obj.nextPageToken;
  }
  request({
    url: api + '/gmail/v1/users/me/' + endpoint + p,
    json: true,
    qs: {
      q: q
    },
    ...
    var messages = body[endpoint].map(function (m) {
      return {
        ...
        body: 'GET ' + api + '/gmail/v1/users/me/' + endpoint + '/' + m.id + '?format=metadata\n'
      }
    })

    messages.length = opts.max || 100
    obj.nextPageToken = body.nextPageToken;
...
Gmail.prototype.messages = function (q, opts) {
  return retrieve(this.key, q, 'messages', opts, this)
}
